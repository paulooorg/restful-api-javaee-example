...
public class GreaterEqual implements PredicateCreator {
	...
	public Predicate create(Filtering filter, CriteriaBuilder criteriaBuilder, Root<?> from) {
		...
		return greaterThanOrEqualTo(criteriaBuilder, field, fieldValue);
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private <Y extends Comparable<? super Y>> Predicate greaterThanOrEqualTo(CriteriaBuilder criteriaBuilder, Expression field, Object value) {
		return criteriaBuilder.greaterThanOrEqualTo(field, (Y) value);
	}
}
