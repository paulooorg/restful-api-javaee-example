...
public class LessEqual implements PredicateCreator {
	...
	public Predicate create(Filtering filter, CriteriaBuilder criteriaBuilder, Root<?> from) {
		...
		return lessThanOrEqualTo(criteriaBuilder, field, fieldValue);
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private <Y extends Comparable<? super Y>> Predicate lessThanOrEqualTo(CriteriaBuilder criteriaBuilder, Expression field, Object value) {
		return criteriaBuilder.lessThanOrEqualTo(field, (Y) value);
	}
}
